...
def groom(_plugin, model):
    ...
    if model[CLUSTER][HARBOR][DISABLED]:
        ...
        setDefaultInMap(model[CLUSTER][HARBOR], SCHEME, "https")
        setDefaultInMap(model[CLUSTER][HARBOR], NOTARY, False)
        setDefaultInMap(model[CLUSTER][HARBOR], TRIVY, False)
        setDefaultInMap(model[CLUSTER][HARBOR], CHARTMUSEUM, False)
        lookupRepository(model, "harbor", repoId = model[CLUSTER][HARBOR][REPO_ID])
        if model[CLUSTER][HARBOR][SCHEME] == "https":
            if SSL_KEY_SRC not in model[CLUSTER][HARBOR] or SSL_CERT_SRC not in model[CLUSTER][HARBOR]:
                ERROR("If 'scheme' == 'ssl', then both 'ssl_cert_src' and 'ssl_key_src' must be defined")
